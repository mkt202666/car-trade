...
SELECT 1, 1, '帕萨特',  1, 'ACTIVE' WHERE NOT EXISTS (SELECT 1 FROM series WHERE id = 1);

INSERT INTO series (id, brand_id, name, sort_order, status)
SELECT 2, 1, '速腾',    2, 'ACTIVE' WHERE NOT EXISTS (SELECT 1 FROM series WHERE id = 2);

INSERT INTO series (id, brand_id, name, sort_order, status)
SELECT 3, 2, '凯美瑞',  1, 'ACTIVE' WHERE NOT EXISTS (SELECT 1 FROM series WHERE id = 3);

INSERT INTO series (id, brand_id, name, sort_order, status)
SELECT 4, 3, 'E级',    1, 'ACTIVE' WHERE NOT EXISTS (SELECT 1 FROM series WHERE id = 4);

INSERT INTO series (id, brand_id, name, sort_order, status)
SELECT 5, 4, '5系',    1, 'ACTIVE' WHERE NOT EXISTS (SELECT 1 FROM series WHERE id = 5);

INSERT INTO series (id, brand_id, name, sort_order, status)
SELECT 6, 5, 'Model Y', 1, 'ACTIVE' WHERE NOT EXISTS (SELECT 1 FROM series WHERE id = 6);
...
INSERT INTO users (id, phone, password, nickname, shop_name, credit_grade, credit_score, deal_count, on_sale_count, certification_status, status)
...
-- =====================================================================
SELECT setval('brands_id_seq',       (SELECT COALESCE(MAX(id), 0) FROM brands),       false);
SELECT setval('series_id_seq',       (SELECT COALESCE(MAX(id), 0) FROM series),       false);
SELECT setval('models_id_seq',       (SELECT COALESCE(MAX(id), 0) FROM models),       false);
SELECT setval('users_id_seq',        (SELECT COALESCE(MAX(id), 0) FROM users),        false);
SELECT setval('car_sources_id_seq',  (SELECT COALESCE(MAX(id), 0) FROM car_sources),  false);
SELECT setval('car_images_id_seq',   (SELECT COALESCE(MAX(id), 0) FROM car_images),   false);
SELECT setval('member_plans_id_seq', (SELECT COALESCE(MAX(id), 0) FROM member_plans), false);
SELECT setval('cities_id_seq',       (SELECT COALESCE(MAX(id), 0) FROM cities),       false);
