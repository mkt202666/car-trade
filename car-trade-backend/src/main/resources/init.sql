...
COMMENT ON COLUMN car_sources.video_url IS '车源视频URL';
...
COMMENT ON COLUMN messages.sender_id IS '发送人ID(系统消息可为NULL)';
...
COMMENT ON COLUMN user_coupons.received_at IS '领取时间(与created_at一致,便于业务字段单独处理)';
COMMENT ON COLUMN user_coupons.created_at IS '创建时间';
...
COMMENT ON COLUMN user_membership.end_at IS '结束时间(用于历史归档或退款标记)';
...
COMMENT ON COLUMN chat_messages.sender_id IS '发送人ID(系统消息可为NULL)';
COMMENT ON COLUMN chat_messages.content IS '消息正文(文本/图片URL/卡片JSON等,按message_type解析)';
COMMENT ON COLUMN chat_messages.message_type IS '消息类型: TEXT-文本, IMAGE-图片, FILE-文件, SYSTEM-系统提示, CARD-卡片消息';
...
-- 说明：
--   以下两个测试账号（默认密码：123456
--     手机号         密码      角色
--     13800138001   123456  诚信车行（车行主）
--     13800138002   123456  车坊张老板
--
--   后端登录逻辑：已有账号若设置了 BCrypt 密码，则需正确密码验证。
--   测试账号密码均为: 123456
...
VALUES (1, '13800138001', '$2a$10$N9qo8uLOickgx2ZMRZoMye.IjzqAKL9xL5jvMFVdNJHvGCgTq/VEq', '诚信车行', '诚信二手车', 'A', 850, 156, 23, 'CERTIFIED', 'ACTIVE');

INSERT INTO users (id, phone, password, nickname, shop_name, credit_grade, credit_score, deal_count, on_sale_count, certification_status, status)
VALUES (2, '13800138002', '$2a$10$N9qo8uLOickgx2ZMRZoMye.IjzqAKL9xL5jvMFVdNJHvGCgTq/VEq', '车坊张老板', '精品车坊', 'S', 920, 328, 45, 'CERTIFIED', 'ACTIVE');
