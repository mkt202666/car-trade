...
// Proxy via Node fetch to bypass server-side 502 on Chromium
...
async function clickLabel(pg, label) {
  const found = await pg.evaluate(target => {
    ...
    return cands[0] ? { x: cands[0].x, y: cands[0].y, t: cands[0].t } : null;
  }, label);
...
async function capture(name, from, action) {
  ...
  console.log(`[${allResults.length}] ${name}: ${d.buttons.length} btns, ${d.inputs.length} inputs`);
  return d;
}
...
if (carHit) {
  await page.mouse.click(carHit.x, carHit.y);
  await sleep(1500);
...
// 发布车源
await goHome();
await clickLabel(page, '发布车源');
await capture('发布车源', '首页-找车', '点击发布车源');
await page.goBack().catch(()=>{}); await sleep(1000);

// 在线客服
await goHome();
await clickLabel(page, '在线客服');
await capture('在线客服', '首页-找车', '点击在线客服');
await page.goBack().catch(()=>{}); await sleep(1000);

// 筛选标签
await goHome();
const filters = ['最新', '新能源', '保证金', '出口'];
let prevFilter = '首页-找车';
for (const f of filters) {
  const h = await clickLabel(page, f);
  if (h) {
    await capture('筛选-' + f, prevFilter, '筛选' + f);
    prevFilter = '筛选-' + f;
  }
}
...
// 交易 tab 切换
const tradeTabs = ['卖出的车', '买到的车', '待确认', '交易中', '争议中', '已完成', '已终止'];
let prevTrade = '交易';
for (const t of tradeTabs) {
  const h = await clickLabel(page, t);
  if (h) {
    await capture('交易-' + t, prevTrade, '切换' + t);
    prevTrade = '交易-' + t;
  }
}
...
const aiButtons = ['新的对话', '历史任务', '查一下奥迪Q5最近的价格走势', '帮我全网搜一下价格低于12万的宝马3系', '帮我自动上传新车源', '今日抢手车源简报分析'];
let prevAI = 'AI助理';
for (const b of aiButtons) {
  const h = await clickLabel(page, b);
  if (h) {
    await capture('AI-' + b.slice(0,15), prevAI, b);
    prevAI = 'AI-' + b.slice(0,15);
    // Close any dialog
...
const msgItems = ['自动推广', '订单状态更新', '新的订单合同', '可用保证金不足', '新成员申请加入您的车行'];
for (const m of msgItems) {
  const h = await clickLabel(page, m);
  if (h) {
    await capture('消息-' + m.slice(0,10), '消息', m);
...
const myItems = ['我的订单', '我的车行', '金融服务', '我的优惠券', '客服支持'];
for (const m of myItems) {
  const h = await clickLabel(page, m);
  if (h) {
    await capture('我的-' + m, '我的', m);
    await page.goBack().catch(()=>{}); await sleep(800);
    await clickLabel(page, '我的'); await sleep(800);
  }
}
...
// 登录页
const loginHit = await clickLabel(page, '登录 / 注册');
if (loginHit) {
  await capture('登录注册', '我的', '点击登录注册');
  await page.goBack().catch(()=>{}); await sleep(800);
  await clickLabel(page, '我的'); await sleep(800);
}

// 认证页
const certHit = await clickLabel(page, '立即认证');
if (certHit) {
  await capture('车商认证', '我的', '点击立即认证');
  await page.goBack().catch(()=>{}); await sleep(800);
}
...
console.log('Report: output/prototype-analysis.json');
console.log('Screenshots: output/prototype-screenshots/');
