...
`car-trade-backend/src/main/resources/` 下包含三份独立的 SQL 脚本，**全部使用 `WHERE NOT EXISTS` / `IF NOT EXISTS` 幂等语法，可重复执行而不报错**。

| 脚本 | 定位 | 执行时机 | 说明 |
| --- | --- | --- | --- |
| `init.sql` | 完整初始化 | 第一次部署时必须执行 | 建库、建表（31 张表）、测试用户 / 品牌 / 车系 / 车型 / 车源 / 车源图片 / 会员套餐 |
| `data-cities.sql` | 行政区划数据 | 第一次部署时执行 | 34 省、333 地级市、部分区县；依赖 `cities` 表（由 `init.sql` 创建） |
| `schema.sql` | 增量迁移与补充测试数据 | `init.sql` 之后按需执行 | 为 `car_sources` 补充出口字段、再写入一批测试品牌 / 车源 / 图片，重置序列 |

**正确执行顺序**：

```
init.sql  →  data-cities.sql  →  schema.sql
```

**运行时自动行为**：

- `spring.sql.init.mode=never`：Spring Boot 原生 SQL 初始化已禁用。
- `StartupRunner`（`car-trade.init-db=true` 时）：只执行轻量级 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` / `CREATE TABLE IF NOT EXISTS`，不读取外部 `.sql` 文件。
- 城市数据、测试车源等业务数据必须通过 `psql -f <script.sql>` 手动加载。
...
COMMENT ON COLUMN car_sources.export_countries IS '出口国家: 多个国家以逗号分隔, 如 "俄罗斯,哈萨克斯坦"';
```

#### 4.4.3 车源图片表 car_images
...
#### 4.4.25 在线客服工单表 customer_service_tickets
...
#### 4.4.26 聊天会话表 chat_conversations
...
#### 4.4.27 聊天会话成员表 chat_conversation_members
